...
const apiKeyCreatedSubject = `An API key for {{.Network.Name}} has been created`

const apiKeyCreatedText = `Dear {{.User.Name}},

A new API key "{{.Key.PrettyName}}" has been issued for "{{.Entity.ID}}" on {{.Network.Name}} with the following rights:
